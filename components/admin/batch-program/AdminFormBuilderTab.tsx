...
import { RefreshCw, BookOpen, Save, X, Edit, Check, Sparkles, AlertCircle, Eye, Info } from 'lucide-react';
import { toast } from 'sonner';
import { cn } from '@/lib/utils';
import { getRegistrationQuestionsAdmin, updateRegistrationQuestion } from '@/app/(protected)/admin/actions';
...
const SECTION_NAMES: Record<number, string> = {
  1: 'Section 1: Komitmen & Pemahaman',
  2: 'Section 2: Izin & Pilihan Program',
  3: 'Section 3: Waktu Setoran',
  4: 'Section 4: Pemahaman Program',
};

export function AdminFormBuilderTab() {
  ...
  const fetchQuestions = async () => {
    setIsLoading(true);
    try {
      const result = await getRegistrationQuestionsAdmin();
      if (result.success && result.data) {
        setQuestions(result.data as Question[]);
      } else {
        toast.error(result.error || 'Gagal memuat pertanyaan pendaftaran');
      }
    } catch (error) {
  ...
  const handleToggleActive = async (id: string, code: string, currentStatus: boolean) => {
    ...
    try {
      ...
      const result = await updateRegistrationQuestion(id, {
      ...
      if (result.success) {
        toast.success(`Pertanyaan "${code}" berhasil ${newStatus ? 'diaktifkan' : 'dinonaktifkan'}`);
      } else {
        // Revert UI if failed
        setQuestions(prev =>
          prev.map(q => (q.id === id ? { ...q, is_active: currentStatus } : q))
        );
        toast.error(result.error || 'Gagal mengubah status pertanyaan');
      }
    } catch (error) {
      // Revert UI if error
      setQuestions(prev =>
  ...
  const handleUpdateQuestion = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const result = await updateRegistrationQuestion(editingQuestion.id, {
        label: editFormData.label,
        description: editFormData.description || null,
        warning_text: editFormData.warning_text || null,
        is_active: editFormData.is_active,
      ...
      if (result.success) {
        toast.success(`Pertanyaan "${editingQuestion.field_key}" berhasil diperbarui!`);
        setEditingQuestion(null);
        fetchQuestions();
      } else {
        toast.error(result.error || 'Gagal memperbarui pertanyaan');
      }
    } catch (error: any) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      {/* Alert Info */}
      <div className="bg-purple-50 border border-purple-200 text-purple-950 p-4 rounded-2xl text-sm flex items-start gap-3 shadow-sm">
        <Sparkles className="w-5 h-5 text-purple-700 flex-shrink-0 mt-0.5" />
        <div className="space-y-1">
          <p className="font-bold">Form Builder Dinamis Pendaftaran Tikrar</p>
          <p className="opacity-90">
            Kelola seluruh teks pertanyaan pendaftaran thalibah, teks deskripsi, petunjuk bantuan, atau kotak peringatan (warning) dari halaman ini. 
            Semua field dipetakan ke dalam database. Toggling Switch status aktif akan langsung menampilkan/menyembunyikan pertanyaan di formulir pendaftaran thalibah.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 flex justify-between items-center gap-4">
        <div className="flex items-center gap-2 text-sm text-gray-500 font-medium">
          Total: <span className="font-bold text-gray-700">{questions.length} Pertanyaan</span> (
          <span className="font-bold text-green-700">{questions.filter(q => q.is_active).length} Aktif</span>,
          <span className="font-bold text-purple-700">{questions.filter(q => q.is_required && q.is_active).length} Wajib</span>)
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => setIsPreviewingAll(true)}
            className="p-2.5 rounded-xl bg-purple-700 hover:bg-purple-800 text-white transition-all flex items-center gap-2 shadow-sm"
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center shadow-sm">
          ...
          <p className="text-gray-500 font-medium">Belum ada pertanyaan terdaftar di database.</p>
        </div>
      ) : (
                        ...
                        <div>
                          ...
                          {q.description && (
                            <p className="text-xs text-gray-500 mt-1 flex items-start gap-1">
                              ...
                              <span>Petunjuk: {q.description}</span>
                            </p>
                          )}
                          {q.warning_text && (
                            <p className="text-xs text-amber-700 mt-1 flex items-start gap-1 bg-amber-50/50 p-2 rounded-lg border border-amber-100/50">
                              ...
                              <span>Peringatan: {q.warning_text}</span>
                            </p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-4 border-t md:border-t-0 pt-3 md:pt-0 border-gray-100 flex-shrink-0 justify-between md:justify-end">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className={cn(
                              "relative inline-flex h-8 w-16 shrink-0 cursor-pointer items-center rounded-full transition-colors duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-purple-600/20 focus:ring-offset-2 p-1",
                              q.is_active ? "bg-purple-600" : "bg-gray-200"
                            )}
                            title={q.is_active ? "Nonaktifkan pertanyaan" : "Aktifkan pertanyaan"}
                          >
                            <span
                              className={cn(
                        ...
                        <button
                          ...
                          className="p-2 rounded-xl border border-gray-200 hover:bg-purple-50 hover:border-purple-200 text-gray-600 hover:text-purple-700 transition-all flex items-center gap-1.5 text-xs font-bold"
                        >
      ...
      {editingQuestion && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-xl w-full max-h-[92vh] overflow-hidden flex flex-col animate-in fade-in zoom-in duration-300">
            {/* Header */}
            <div className="bg-gradient-to-r from-purple-900 to-purple-800 p-6 flex items-center justify-between text-white relative">
              <button
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <h2 className="text-xl font-bold">Edit Pertanyaan</h2>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleUpdateQuestion} className="flex-1 overflow-y-auto">
              <div className="p-6 space-y-4">
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5">Teks Pertanyaan / Label</label>
                  <textarea
                    required
                    value={editFormData.label}
                    onChange={(e) => setEditFormData({ ...editFormData, label: e.target.value })}
                    rows={3}
                    className="w-full px-3 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-purple-600/20 focus:border-purple-700 text-sm"
                  />
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-purple-600/20 focus:border-purple-700 text-sm bg-white"
                  >
                ...
                <div>
                  ...
                  {['radio', 'checkbox'].includes(editFormData.input_type) && (
                    <p className="text-[10px] text-purple-600 font-bold mb-1.5">
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 mb-2 p-1.5 bg-gray-50 border border-gray-200 rounded-xl">
                    ...
                    <button
                      ...
                      className="px-2 py-1 bg-white hover:bg-purple-50 border border-gray-200 hover:border-purple-200 rounded-lg text-xs font-bold text-gray-700 hover:text-purple-700 transition-all shadow-sm"
                      title="Bullet point"
                    >
                      • Bullet
                    </button>
                  ...
                  <textarea
                    ...
                    className="w-full px-3 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-purple-600/20 focus:border-purple-700 text-sm font-medium"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Akan ditampilkan sebagai kotak alert kuning..."
                    className="w-full px-3 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-purple-600/20 focus:border-purple-700 text-sm"
                  />
                </div>
                ...
                {['radio_options', 'select', 'multi_select'].includes(editFormData.input_type) && (
                  <div className="space-y-3 pt-2">
                          ...
                          <div key={idx} className="flex items-center gap-2 bg-white p-2 rounded-lg border border-gray-200">
                            <div className="flex-1 space-y-1">
                              <input
                                ...
                                className="w-full px-2 py-1 border border-gray-200 rounded text-xs focus:outline-none focus:border-purple-700"
                              />
                              <input
                                type="text"
                                ...
                                className="w-full px-2 py-1 border border-gray-200 rounded text-[10px] font-mono focus:outline-none focus:border-purple-700 bg-gray-50/50"
                              />
                            </div>
                            <button
                              type="button"
                    ...
                    <button
                      ...
                      className="w-full py-2 bg-purple-50 hover:bg-purple-100 border border-dashed border-purple-200 hover:border-purple-300 rounded-xl text-xs font-bold text-purple-700 transition-all flex items-center justify-center gap-1.5"
                    >
          ...
          <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[92vh] overflow-hidden flex flex-col animate-in zoom-in duration-300">
            ...
            <div className="bg-gradient-to-r from-purple-900 to-purple-800 p-6 flex items-center justify-between text-white relative">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="text-[10px] font-bold text-yellow-300 uppercase tracking-[0.2em] mb-0.5">
                    Pratinjau Formulir Publik (Tikrar Tahfidz)
                  </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-gray-50/50 space-y-6">
              {[1, 2, 3, 4].map((sectionNum) => {
                ...
                return (
                  <div key={sectionNum} className="space-y-4">
                    <div className="bg-purple-100/50 text-purple-950 border border-purple-200/50 rounded-xl px-4 py-2 text-xs font-bold uppercase tracking-wider">
                      {SECTION_NAMES[sectionNum]}
                    </div>
            ...
            <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex items-center justify-end">
              <button
                type="button"
                onClick={() => setIsPreviewingAll(false)}
                className="px-5 py-2.5 rounded-xl bg-purple-700 hover:bg-purple-800 text-white text-xs font-bold transition-all shadow-sm"
              >
                Tutup Pratinjau
              </button>
            </div>
          ...
          <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full max-h-[92vh] overflow-hidden flex flex-col animate-in fade-in zoom-in duration-300">
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-gray-50/50">
              <div className="space-y-4">
                ...
                {/* Info Metadata */}
                <div className="bg-purple-50/50 border border-purple-100/50 rounded-xl p-3.5 space-y-1.5 text-xs text-purple-900">
                  <div className="font-bold flex items-center gap-1.5 mb-1 text-purple-950">
                    <Sparkles className="w-3.5 h-3.5 text-purple-700" />
                    Detail Parameter Database:
                  </div>
                  <div><span className="font-semibold">Nama Field:</span> <code className="bg-purple-100/60 px-1 py-0.5 rounded font-mono text-[11px] text-purple-800">{previewQuestion.field_key}</code></div>
                  ...
                  <div><span className="font-semibold">Format Jawaban:</span> <code className="bg-purple-100/60 px-1 py-0.5 rounded font-mono text-[11px] text-purple-800">{previewQuestion.input_type || 'text'}</code></div>
                </div>
              </div>
            </div>
            ...
            <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex items-center justify-end">
              <button
                type="button"
                onClick={() => setPreviewQuestion(null)}
                className="px-4 py-2 rounded-xl bg-purple-700 hover:bg-purple-800 text-xs font-bold text-white transition-all shadow-sm"
              >
                Tutup Pratinjau
              </button>
            </div>
    ...
    <div className="mt-3 space-y-2">
      ...
      {inputType === 'file' && (
        <div className="border-2 border-dashed border-gray-200 rounded-2xl p-6 text-center bg-gray-50">
          <div className="mx-auto w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center mb-2">

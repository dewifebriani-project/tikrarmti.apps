-- ============================================================================
-- Debug: Check vivinurvina@gmail.com (muallimah who also registered as thalibah)
-- ============================================================================
...
WITH user_info AS (
    SELECT id FROM auth.users WHERE email = 'vivinurvina@gmail.com'
)
SELECT
...
WHERE email ILIKE 'vivinurvina@gmail.com'
...
WHERE u.email = 'vivinurvina@gmail.com';

-- 6. Summary: What's the problem?
WITH user_info AS (
    SELECT id, email FROM auth.users WHERE email = 'vivinurvina@gmail.com'
)
SELECT
    'DIAGNOSIS' as type,
    CASE
        ...
        ) THEN 'user_id MISMATCH between auth.users and pendaftaran_tikrar_tahfidz - THIS IS THE PROBLEM!'
        ELSE 'Everything looks OK - user should have access'
    END as diagnosis;

-- 7. If mismatch detected, show the fix
WITH user_info AS (
    SELECT id, email FROM auth.users WHERE email = 'vivinurvina@gmail.com'
)
SELECT
    ...
    'UPDATE pendaftaran_tikrar_tahfidz SET user_id = ''' || u.id || ''' WHERE id = ''' || p.id || ''';' as sql_fix
FROM pendaftaran_tikrar_tahfidz p
CROSS JOIN user_info u
WHERE p.email ILIKE u.email AND p.user_id != u.id;

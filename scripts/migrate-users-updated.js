/**
 * MIGRASI USERS TABLE - Updated dengan semua kolom
 * Convert CSV ke SQL INSERT statements
 *
 * Run: node scripts/migrate-users-updated.js
 */
...
for (let i = 1; i < lines.length; i++) {
  ...
  const id = escapeSQL(values[colIndex.id]);
  ...
  const current_tikrar_batch_id = values[colIndex.current_tikrar_batch_id] && values[colIndex.current_tikrar_batch_id] !== ''
    ...
    : 'NULL';
  const name = values[colIndex.name] ? escapeSQL(values[colIndex.name]) : 'NULL';
  const is_blacklisted = (values[colIndex.is_blacklisted] === 'true' || values[colIndex.is_blacklisted] === 't') ? 'true' : 'false';
  const blacklist_reason = values[colIndex.blacklist_reason] ? escapeSQL(values[colIndex.blacklist_reason]) : 'NULL';
  const blacklisted_at = values[colIndex.blacklisted_at] ? escapeSQL(values[colIndex.blacklisted_at]) : 'NULL';
  const blacklist_notes = values[colIndex.blacklist_notes] ? escapeSQL(values[colIndex.blacklist_notes]) : 'NULL';
  const blacklist_by = values[colIndex.blacklist_by] && values[colIndex.blacklist_by] !== ''
    ? escapeSQL(values[colIndex.blacklist_by])
    : 'NULL';

  const insertStmt = `INSERT INTO public.users (id, email, password_hash, full_name, role, avatar_url, is_active, created_at, updated_at, provinsi, kota, alamat, whatsapp, telegram, zona_waktu, tanggal_lahir, tempat_lahir, pekerjaan, alasan_daftar, jenis_kelamin, negara, nama_kunyah, roles, current_tikrar_batch_id, name, is_blacklisted, blacklist_reason, blacklisted_at, blacklist_notes, blacklist_by) VALUES (${id}, ${email}, ${password_hash}, ${full_name}, ${role}, ${avatar_url}, ${is_active}, ${created_at}, ${updated_at}, ${provinsi}, ${kota}, ${alamat}, ${whatsapp}, ${telegram}, ${zona_waktu}, ${tanggal_lahir}, ${tempat_lahir}, ${pekerjaan}, ${alasan_daftar}, ${jenis_kelamin}, ${negara}, ${nama_kunyah}, ${roles}, ${current_tikrar_batch_id}, ${name}, ${is_blacklisted}, ${blacklist_reason}, ${blacklisted_at}, ${blacklist_notes}, ${blacklist_by}) ON CONFLICT (id) DO NOTHING;`;
...
// Write to SQL file
const sqlContent = `-- =====================================================
-- MIGRASI USERS TABLE - Complete dengan semua kolom
...
console.log(`\nNext steps:`);
console.log(`1. Run scripts/03_create_users_table.sql in project BARU`);
console.log(`2. Copy contents of ${outputPath}`);
console.log(`3. Paste in Supabase SQL Editor (project BARU)`);

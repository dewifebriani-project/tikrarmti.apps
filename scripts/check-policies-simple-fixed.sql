...
SELECT
  ...
  SUM(CASE WHEN permissive::text = 'true' THEN 1 ELSE 0 END) as permissive_policies,
  SUM(CASE WHEN permissive::text = 'false' THEN 1 ELSE 0 END) as restrictive_policies,
  STRING_AGG(policyname, ', ' ORDER BY policyname) as all_policy_names,
  CASE
    WHEN BOOL_AND(permissive::text = 'true') THEN 'All PERMISSIVE (need at least one to pass)'
...
SELECT
  ...
  END as policy_type,
  CASE
    WHEN permissive::text = 'true' THEN 'At least one must pass'
    ELSE 'All must pass'
  END as rule,

-- =====================================================
-- SETUP SCHEMA - TIKRAR MTI APPS
-- Jalankan di PROJECT BARU: https://lhqbqzrghdbbmstnhple.supabase.co
-- Order: Buat tabel dulu, baru add foreign keys
-- =====================================================

-- =====================================================
-- BAH 1: Create Tabel Utama (tanpa foreign keys dulu)
-- =====================================================
...
CREATE TABLE IF NOT EXISTS public.users (
  ...
  pekerjaan text,
  alasan_daftar text,
  jenis_kelamin character varying CHECK (jenis_kelamin IS NULL OR jenis_kelamin IN ('Perempuan', 'Laki-laki')),
  negara character varying,
  nama_kunyah text,
  roles text[] CHECK (roles <@ ARRAY['admin'::text, 'calon_thalibah'::text, 'thalibah'::text, 'muallimah'::text, 'musyrifah'::text, 'pengurus'::text]),
...
CREATE TABLE IF NOT EXISTS public.batches (
  ...
  status character varying DEFAULT 'draft' CHECK (status::text = ANY (ARRAY['draft'::text, 'open'::text, 'closed'::text, 'archived'::text)),
  created_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP,
  updated_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS public.programs (
  ...
  status character varying DEFAULT 'draft' CHECK (status::text = ANY (ARRAY['draft'::text, 'open'::text, 'ongoing'::text, 'completed'::text, 'cancelled'::text)),
  created_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP,
  updated_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP,
  class_type text CHECK (class_type = ANY (ARRAY['tikrar_tahfidz'::text, 'pra_tahfidz'::text, 'tashih_only'::text, 'ujian_only'::text, 'tashih_ujian'::text]))
);
...
CREATE TABLE IF NOT EXISTS public.halaqah (
  ...
  status character varying DEFAULT 'active' CHECK (status::text = ANY (ARRAY['active'::text, 'inactive'::text, 'suspended'::text)),
  created_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP,
  updated_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP,
  zoom_link text,
  muallimah_id uuid,
...
CREATE TABLE IF NOT EXISTS public.halaqah_students (
  ...
  assigned_by uuid,
  status character varying DEFAULT 'active' CHECK (status::text = ANY (ARRAY['active'::text, 'transferred'::text, 'graduated'::text, 'dropped'::text))
);
...
CREATE TABLE IF NOT EXISTS public.halaqah_mentors (
  ...
  role character varying NOT NULL CHECK (role::text = ANY (ARRAY['ustadzah'::text, 'musyrifah'::text)),
...
CREATE TABLE IF NOT EXISTS public.pendaftaran_tikrar_tahfidz (
  ...
  status character varying DEFAULT 'pending' CHECK (status::text = ANY (ARRAY['pending'::text, 'approved'::text, 'rejected'::text, 'withdrawn'::text])),
  selection_status character varying DEFAULT 'pending' CHECK (selection_status::text = ANY (ARRAY['pending'::text, 'selected'::text, 'not_selected'::text, 'waitlist'::text)),
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS public.daftar_ulang_submissions (
  ...
  status character varying NOT NULL DEFAULT 'draft' CHECK (status::text = ANY (ARRAY['draft'::character varying, 'submitted'::character varying, 'approved'::character varying, 'rejected'::character varying]::text[])),
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.muallimah_registrations (
  ...
  status text NOT NULL DEFAULT 'pending' CHECK (status = ANY (ARRAY['pending'::text, 'review'::text, 'approved'::text, 'rejected'::text, 'waitlist'::text])),
  created_at timestamp with time zone DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.exam_attempts (
  ...
  status text DEFAULT 'in_progress' CHECK (status = ANY (ARRAY['in_progress'::text, 'submitted'::text, 'graded'::text])),
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now()
);
...
-- =====================================================
-- BAH 2: Add Foreign Keys (setelah semua tabel ada)
-- =====================================================
...
-- halaqah_students -> halaqah, users
...
-- pendaftaran -> users, batches, programs
...
-- daftar_ulang -> users, pendaftaran, batches
...
-- musyrifah_registrations -> users, batches
...
-- exam_attempts -> users, pendaftaran
...
-- users -> batches (current_tikrar_batch_id)

    ...
    jurnalRecords.forEach(record => {
        if (record.blok) {
            ...
            blokCodes.forEach(rawBlokCode => {
                ...
                // DEBUG: Check specific blocks
                if (blokCode.startsWith('H1A') || blokCode.startsWith('H2')) {
                    console.log(`Processing Record: ${blokCode} -> Match Found in Map? ${!!current}`);
                }
...
console.log(`Generated ${allBlocks.length} blocks for Juz 30A`);

const weeklyStatus = calculateWeeklyStatus(allBlocks, aamRecords);

console.log("\n--- WEEKLY STATUS ---");
weeklyStatus.slice(0, 4).forEach(w => {
    console.log(`Week ${w.week_number}: ${w.completed_blocks}/${w.total_blocks} (${w.is_completed ? 'PASS' : 'FAIL'})`);
});

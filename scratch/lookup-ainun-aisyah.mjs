...
async function main() {
  ...
  console.log('\n=== 3. Registrasi & Daftar Ulang Ainun ===');
  ...
  console.log('\n=== 5. Cari Aisyah di auth.users ===');
  const { data: authList, error: e3 } = await supabase.auth.admin.listUsers();
  const matchAuth = authList?.users?.filter(u => (u.email || '').toLowerCase().includes('aisyah2020'));
  ...
  const matchAuthTarget = authList?.users?.filter(u => (u.email || '').toLowerCase().includes('abdaish'));

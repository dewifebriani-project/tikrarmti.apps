...
async function run() {
  const email = 'temp-juz-migration-admin@markaztikrar.id';
  ...
  try {
    ...
    if (findError || !publicUser) {
      ...
      const { error: insertError } = await supabaseAdmin
        ...
        .insert({
          id: tempUser.id,
          email: email,
          full_name: 'Temp Juz Migration Admin',
          role: 'admin',
          roles: ['admin'],
    ...
    const sql = fs.readFileSync(path.join(process.cwd(), 'supabase/migrations/20260620_seed_all_juz_options.sql'), 'utf8');

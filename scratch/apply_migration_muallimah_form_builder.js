...
async function run() {
  const email = 'temp-muallimah-migration-admin@markaztikrar.id';
  ...
  try {
    ...
    if (findError || !publicUser) {
      ...
      const { error: insertError } = await supabaseAdmin
        ...
        .insert({
          id: tempUser.id,
          email: email,
          full_name: 'Temp Muallimah Migration Admin',
          role: 'admin',
          roles: ['admin'],
    ...
    const sql = fs.readFileSync(path.join(process.cwd(), 'supabase/migrations/20260620_create_muallimah_registration_questions.sql'), 'utf8');
